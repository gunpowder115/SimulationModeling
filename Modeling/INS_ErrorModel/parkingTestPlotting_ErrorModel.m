...
grid on; hold on;
plot(simTime, p_kalman(2, :), 'r--');
plot(simTime, -p_kalman(2, :), 'r--');
title('Прогноз СКО и реальная ошибка оценивания по координате X1');
subplot(1, 2, 2);
plot(simTime, lat_error_hat - lat_error, 'b');
grid on; hold on;
plot(simTime, p_kalman(3, :), 'r--');
plot(simTime, -p_kalman(3, :), 'r--');
title('Прогноз СКО и реальная ошибка оценивания по координате X2');

figure;
subplot(1, 2, 1);
plot(simTime, vel_E_error_hat - vel_E_error, 'b');
grid on; hold on;
plot(simTime, p_kalman(5, :), 'r--');
plot(simTime, -p_kalman(5, :), 'r--');
title('Прогноз СКО и реальная ошибка оценивания по скорости X3');
subplot(1, 2, 2);
...
title('Прогноз СКО и реальная ошибка оценивания по скорости X3');
xlabel('Время, сек');
ylabel('Ошибка оценивания, м/с');

figure;
plot(simTime, vel_N_error_hat - vel_N_error, 'b');
grid on; hold on;
plot(simTime, p_kalman(5, :), 'r--');
plot(simTime, -p_kalman(5, :), 'r--');
title('Прогноз СКО и реальная ошибка оценивания по скорости X4');
xlabel('Время, сек');
ylabel('Ошибка оценивания, м/с');
